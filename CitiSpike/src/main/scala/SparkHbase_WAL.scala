...
object SparkHbase_WAL {
  ...
  def main(args: Array[String]): Unit = {
    ...
    keys.map(key => {
             ...
             val hbaseRdd: RDD[(ImmutableBytesWritable, Put)] = filteredRdd.map(row => {
               //val table = new HTable(getHBaseConfiguration(),"Positions")
              val rowkey = (row.ACCT_KEY+"_"+row.POSN).getBytes

              val putObj = new Put(rowkey)

              val columnNames = ("account_number"+","+"pp_code"+","+"position"+","+"amount").split(",")
              val dataValues = (row.ACCT_KEY+","+row.PP_CODE+","+row.POSN+","+row.AMOUNT).split(",")

              for(i<-1 to 3){

                putObj.addColumn("position".getBytes,columnNames(i).getBytes,dataValues(i).getBytes)

              }

              Tuple2[ImmutableBytesWritable,Put](new ImmutableBytesWritable(),putObj)

            })

      println("Created Put Object for Key : "+key +" at : "+System.currentTimeMillis())

      println("Data flushed for Key : "+key+ "  at : "+System.currentTimeMillis())
      val conf = getHBaseConfiguration()
      println("Before Saving RDD for Key : "+key+" is : "+System.currentTimeMillis())
      hbaseRdd.saveAsNewAPIHadoopDataset(conf)
      println("End time for Key : "+key+" is : "+System.currentTimeMillis() )
